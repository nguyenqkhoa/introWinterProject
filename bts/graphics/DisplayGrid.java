...
public class DisplayGrid extends GridPane {
	 ...
	 * empty because it is meant to be overridden by child classes
	...
	public void refresh() {
		clear();
		for (int i = 0; i < 100; i++) {
			if (data.getEnemyPlayer().sqHit.contains(i) && data.getTurnPlayer().sqNotHit.contains(i)) {
				this.highlight(Color.RED, i % 10 + 1, i / 10 + 1);
			} else if (data.getTurnPlayer().sqNotHit.contains(i)) {
				this.highlight(Color.BLUE, i % 10 + 1, i / 10 + 1);
			} else if (data.getEnemyPlayer().sqHit.contains(i)) {
